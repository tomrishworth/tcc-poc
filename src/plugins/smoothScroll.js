...
export function smoothScroll(imagesLoaded) {
  const MathUtils = {
  ...
  class Item {
    ...
    layout() {
      // scale the image
      this.DOM.image.style.transform = `scale3d(${this.renderedStyles.imageScale.previous},${
  ...
  class SmoothScroll {
    constructor() {
      ...
      [...this.DOM.content.querySelectorAll('.section')].forEach(item => this.items.push(new Item(item)));
  ...
  const preloadImages = () => {
    return new Promise((resolve, reject) => {
      imagesLoaded(document.querySelectorAll('.section-image'), { background: true }, resolve);
    });
  };
